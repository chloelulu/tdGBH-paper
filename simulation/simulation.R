...
### ---- independent-20 outcome - 50feature(reviewer response for regresssion based methods) ----
## outcome
for(iter in 1:100){
  for(affected.outcome in 0.4){
    for(affected.feature in 1){ 
      for(signal.density in c(0.1)){
        for(outcome in c(20)){
          for(feature in c(50)){
            cat('---------------------\n')
            feature.no = feature * outcome; signals <- feature.no * signal.density; space = feature.no * affected.feature * affected.outcome
            ...
            sim <- simulata.data(outcome =outcome, feature.no = feature.no, cor.struct ='None',signal.strength = 2,
                                 feature.density=affected.feature,signal.densities = outcome.signal.densities)
            cat('[',mean(colSums(sim$truth.mat)>0),';');cat(mean(rowSums(sim$truth.mat)>0),';');cat(sum(sim$truth.mat),']\n')
            save(sim, file = paste0('setting01LMo_small/iter',iter,'_AO',affected.outcome,'_AF',affected.feature,'_SD',signal.density,'_o',outcome,'_f',feature,'.RData'))
          }
        }
      }
    }
        ...
        for(outcome in c(20)){
          for(feature in c(50)){
            ...
            sim <- simulata.data(outcome =outcome, feature.no = feature.no,cor.struct ='None',signal.strength = 2,
                                 feature.density=affected.feature, signal.densities = outcome.signal.densities)
            cat('[',mean(colSums(sim$truth.mat)>0),';');cat(mean(rowSums(sim$truth.mat)>0),';');cat(sum(sim$truth.mat),']\n')
            save(sim, file = paste0('setting01LMg_small/iter',iter,'_AO',affected.outcome,'_AF',affected.feature,'_SD',signal.density,'_o',outcome,'_f',feature,'.RData'))
          }
        }
      }
    }
...
for(iter in 1:100){
  for(affected.outcome in 0.4){
    for(affected.feature in 0.4){
      for(signal.density in c(0.04)){ 
        for(outcome in c(20)){
          for(feature in c(50)){
            cat('---------------------\n')
            feature.no = feature * outcome; signals <- feature.no * signal.density; space = feature.no * affected.feature * affected.outcome
            ...
            sim <- simulata.data(outcome =outcome, feature.no = feature.no,cor.struct ='None',signal.strength = 2,
                                 feature.density=affected.feature, signal.densities = outcome.signal.densities)
            cat('[',mean(colSums(sim$truth.mat)>0),';');cat(mean(rowSums(sim$truth.mat)>0),';');cat(sum(sim$truth.mat),']\n')
            save(sim, file = paste0('setting01LMb_small/iter',iter,'_AO',affected.outcome,'_AF',affected.feature,'_SD',signal.density,'_o',outcome,'_f',feature,'.RData'))
          }
        }
      }
    }
